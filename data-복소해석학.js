...
$$\\int_{-\\infty}^{\\infty}\\frac{xe^{ibx}}{x^2+a^2}dx=2\\pi i\\cdot\\frac{e^{-ab}}{2}=\\pi i\\,e^{-ab}$$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'$f(z)=g(z)/h(z)$에서 $h(z0)=0, h\'(z0)≠0$인 단순극이면 $Res=g(z0)/h\'(z0).$'}
  ]
},
...
$$\\int_C\\frac{z^3f'(z)}{f(z)}dz=2\\pi i(1-1-1)=2\\pi i(-1)=-2\\pi i$$`}
  ...
  answer:'$-2\\pi i$',
  footnotes:[
    {n:'①',text:'$f$가 $z0$에서 $m$차 영점이면 $f\'/f$는 $z0$에서 단순극이고 유수는 $m ($편각원리의 기초).'}
  ]
},
...
$$u(-1,1)=1+2+2-4-6=-5$$`}
  ...
  footnotes:[
    {n:'①',text:'조화함수의 평균값 성질: 중심 $z0,$ 반지름 $r$인 원 위 평균값 $= u(z0).$'},
