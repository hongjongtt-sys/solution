...
window.SUBJECTS['복소해석학'] = {
  ...
  freqGroups: [
    {
      id:'entire', name:'해석함수의 성질 — 정함수·조화함수$·CR$방정식', stars:'★★★★★★', count:9, color:'#4a2d70',
...
$$T^{-1}(2)=-\\frac{i}{\\pi}\\bar z+\\ker(T)=\\left\\{-\\frac{i}{\\pi}\\bar z+b_1z+b_2e^z+b_3(\\bar z-e^{\\bar z})\\ \\middle|\\ b_1,b_2,b_3\\in\\mathbb{C}\\right\\}$$`}
  ...
  footnotes:[
    {n:'①',text:'코시의 적분정리$: f$가 단순연결영역에서 해석적이면 그 안의 임의의 닫힌곡선에서 $∮f dz=0.$'},
    {n:'②',text:'$∮_{|z|=1} 1/z dz = 2πi$는 유수정리의 가장 기본적인 예.'},
...
$$\\int_{-\\infty}^{\\infty}\\frac{xe^{ibx}}{x^2+a^2}dx=2\\pi i\\cdot\\frac{e^{-ab}}{2}=\\pi i\\,e^{-ab}$$`}
  ...
  answer:'$\\pi i\\,e^{-ab}$',
  footnotes:[
    {n:'①',text:'조던 부등식$(Jordan\'s inequality): 0≤t≤π/2$에서 $sin t ≥ 2t/π$이므로 $∫₀^π e^{-R sin t}dt < π/R.$'},
    {n:'②',text:'$f(z)=g(z)/h(z)$에서 $h(z_0)=0, h\'(z_0)≠0$인 단순극이면 $Res=g(z_0)/h\'(z_0).$'}
  ]
},
...
$$f'(1)=(0+1)+i(-2-1)=1-3i$$`}
  ...
  answer:'$n=1$, $f\'(1)=1-3i$',
  footnotes:[
    {n:'①',text:'$f$가 해석적이면 $u,v$는 조화함수(라플라스 방정식 $u_xx+u_yy=0$ 만족).'},
    {n:'②',text:'코시-리만 방정식$: u_x=v_y, u_y=-v_x. f\'(z)=u_x+iv_x$로 계산 가능.'}
  ]
},
{
...
$$f\\!\\left(\\frac{1+i}{3}\\right)=1$$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'해석함수 $g$가 $g(a)=0$이면 $g(z)=(z-a)k(z)$ 꼴로 인수분해되며 $k$도 해석적 (테일러 전개에서 상수항이 0).'},
    {n:'③',text:'최대계수원리: 유계 닫힌영역에서 해석적인 함수의 $|f|$는 경계에서 최댓값을 가진다.'},
    {n:'④',text:'최대계수원리의 등호(강한) 형태$: |f|$가 정의역 내부의 한 점에서 최댓값을 가지면 $f$는 그 정의역에서 상수.'}
  ]
},
{
...
$$\\int_C\\bar z\\,dz=i\\pi+0=i\\pi$$`},
  ...
  answer:'$\\pi$',
  footnotes:[
    {n:'①',text:'$z=x+iy$일 때 $z²=(x²-y²)+2ixy$이므로 실수부는 $x²-y²,$ 허수부는 $2xy.$ 문제의 피적분함수를 $z², z̄$의 조합으로 바꾸면 정함수 부분$(z²)$은 적분정리로 바로 0이 되어 계산이 쉬워진다.'},
...
$2\\pi\\,u(0,0)=6\\pi\\Rightarrow u(0,0)=3$. 그런데 $u(0,0)=C-e^0(0+0)=C$이므로 $C=3$.`}
  ...
  footnotes:[
    ...
    {n:'②',text:'조화함수의 평균값 성질$: u(z_0)=(1/2π)∫₀^{2π}u(z_0+re^{iθ})dθ.$ 해석함수의 실부/허부는 조화함수.'}
  ]
},
{
...
$$|4-(-1)|+3=5+3=8$$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'점 $P$에서 중심 $O,$ 반지름 $r$인 원 위의 점까지 거리의 최댓값은 $|OP|+r,$ 최솟값은 $|OP|-r ($단$, |OP|≥r).$'}
  ]
},
{
...
$$|f(z)|^2_{\\min}=\\frac9{16}\\Rightarrow|f(z)|_{\\min}=\\frac34$$`}
  ...
  footnotes:[
    {n:'①',text:'$|z|=r$인 원 위의 점은 $z=re^{iθ} (θ$는 $0~2π)$로 매개변수화한다. 이렇게 두면 $f(z)$를 삼각함수의 실수부·허수부로 분리해 $|f(z)|²=($실수부)²+(허수부)²을 $θ$의 함수로 정리할 수 있다.'},
    {n:'②',text:'$cos²θ+sin²θ=1$을 이용해 하나의 삼각함수(여기선 $cos²θ)$로 통일하면$, cos²θ$의 범위 [0,1]만 보고 최댓값·최솟값을 바로 구할 수 있다.'}
  ]
},
{
...
$$\\int_C\\frac{z^3f'(z)}{f(z)}dz=2\\pi i(1-1-1)=2\\pi i(-1)=-2\\pi i$$`}
  ...
  answer:'$-2\\pi i$',
  footnotes:[
    {n:'①',text:'$f$가 $z_0$에서 $m$차 영점이면 $f\'/f$는 $z_0$에서 단순극이고 유수는 $m$ (편각원리의 기초).'}
  ]
},
...
$$f''\\left(\\frac\\pi2\\right)=-9e^{3i\\pi/2}+8=-9(-i)+8=8+9i$$`}
  ...
  footnotes:[
    {n:'①',text:'함수 $f=u+iv$가 해석적(정함수)이려면 $u,v$가 코시-리만 방정식$(u_x=v_y, u_y=-v_x)$을 만족해야 하고, 그 결과로 $u,v$는 조화함수(라플라스 방정식 만족)가 된다. 즉 $u_xx+u_yy=0$은 해석성의 필요조건.'},
    {n:'②',text:'$f(z)=e^{3iz}+4z²$ 처럼 $u,v$를 하나의 복소식으로 합치면 $f\'(z), f\'\'(z)$는 실변수 미분과 같은 규칙으로 바로 계산할 수 있다 (지수함수·다항식 미분 공식 그대로 적용).'}
  ]
},
...
$$u(-1,1)=1+2+2-4-6=-5$$`}
  ...
  footnotes:[
    {n:'①',text:'조화함수의 평균값 성질: 중심 $z_0$, 반지름 $r$인 원 위 평균값 $= u(z_0)$.'},
...
$$\\int_C\\frac1{(z-3)(z^3-z-4)}dz=2\\pi i\\left(-\\frac1{20}\\right)=-\\frac{\\pi i}{10}$$`}
  ...
  footnotes:[
    {n:'①',text:'루셰 정리$: γ$와 그 내부에서 $f,g$가 해석적이고 $γ$ 위에서 $|g|<|f|$이면 $f+g$와 $f$는 $γ$ 내부에서 같은 개수의 영점을 갖는다.'},
    {n:'②',text:'$f(z)$가 무한대 근방에서 $zf(z)→0$이면(즉 $O(1/z²)$ 이상으로 감소) 복소평면 위 모든 극점에서의 유수의 합은 0이다.'}
  ]
},
...
// ───────────────────────────────────────
{
  ...
  answer:'$4\\pi i$',
  footnotes:[
    {n:'①',text:'$|z|=1 ($단위원) 위에서는 $z z̄=|z|²=1$이 항상 성립하므로 $z̄=1/z.$ 이 관계를 알면 굳이 매개변수화하지 않고도 적분을 $1/z, z$ 형태로 바꿔 유수정리로 바로 계산할 수도 있다.'},
    {n:'②',text:'$dz̄$는 $z̄$를 곡선의 매개변수(여기선 $θ)$에 대해 미분한 것$. z=e^{iθ}$이면 $z̄=e^{-iθ}$이므로 $dz̄=-ie^{-iθ}dθ$로$, dz=ie^{iθ}dθ$와 부호·형태가 다르다는 점에 주의.'}
  ]
},
{
...
$$\\frac{f'(1-i)}{f(1+i)}=\\frac{\\pi e^\\pi}{e^\\pi}=\\pi$$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'$f(z)=a₀cosh(πz)+ā₀sinh(πz)$에서 $f\'(i)=π[a₀sinh(iπ)+ā₀cosh(iπ)]=-πā₀ (sinh(iπ)=0,cosh(iπ)=-1). a₀=-1$이면 $-π(-1)=π$로 일치.'}
  ]
},
...
$z^{-1}$항($n=1$)의 계수(유수)는 $-1$.`},
  ...
  footnotes:[
    {n:'①',text:'$z=0$은 $e^{-1/z}$의 본질적 특이점$(essential singularity)$이다. 이런 점 주변에서는 유한 차수의 극이 아니라 무한히 많은 음수 차수 항$(z⁻¹,z⁻²,...)$을 갖는 로랑 급수로 전개된다.'},
...
| $10,11,\\ldots$ | $9$ | $6+3+5=14$ | $-5$ |`},
  ...
  footnotes:[
    {n:'①',text:'편각원리$: f$가 $γ$ 위에서 영점·극점이 없고 내부에서 유리형이면 $∮(f\'/f)dz=2πi(Z-P) (Z,P$는 중복도 포함 영점·극점 개수).'}
  ]
},
{
