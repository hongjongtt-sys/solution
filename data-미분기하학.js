...
따라서 $\\tau_\\beta=0$.`},
  ...
  answer:'$\\sqrt{1+\\tau^2}$',
  footnotes:[
    {n:'①',text:'프레네-세레 공식$: T\'=κN, N\'=-κT+τB, B\'=-τN. (T,N,B$는 단위접선·주법선·종법선벡터)'},
...
$$\\oint_{\\partial S}\\kappa_g\\,ds=2\\pi-\\pi-\\pi\\left(1-\\frac1{\\sqrt2}\\right)=\\frac{\\pi}{\\sqrt2}$$`}
  ...
  footnotes:[
    {n:'①',text:'회전면에서 자오선$(meridian)$과 위선$(parallel)$은 항상 서로 직교 $(F=0$인 정칙 매개변수화의 표준적 성질).'},
    {n:'②',text:'가우스-보네 정리: 원판형(단순연결) 영역 $S$에서 $∬K dA + ∮κ_g ds + Σ$외각 $= 2π.$'}
  ]
},
...
$$\\kappa=\\frac{16\\sqrt{15}}{75}$$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'꼭짓점에서 모선을 따라 거리 $ρ$인 점에서, 모선에 수직한 방향의 주곡률은 $cosα/(ρsinα) ($원뿔을 국소적 회전면으로 보고 유도).'},
...
$\\kappa$가 상수이므로 $s=1$에서도 동일.`}
  ...
  footnotes:[
    {n:'①',text:'평면곡선(한 평면 안에 있는 곡선)은 항상 비틀림률이 0이다 (역도 성립$: τ≡0$이면 평면곡선).'},
    {n:'②',text:'길이함수 $L(t)=∫₀ᵗ|β\'(s)|ds$가 모든 $t$에서 $3t$와 같으면, 양변을 미분하여 피적분함수 자체가 상수 3임을 얻는다(미적분학의 기본정리).'}
  ]
},
{
...
$$\\kappa_g^2=\\frac54-\\frac14=1\\ \\Rightarrow\\ |\\kappa_g|=1$$`}
  ...
  footnotes:[
    {n:'①',text:'반지름 $R$인 구는 전제점(모든 방향의 법곡률이 같은 점)으로만 이루어져 있고, 그 값은 항상 $1/R.$'}
  ]
},
...
$$=1-4\\kappa(2)=0$$`},
  ...
  footnotes:[
    ...
    {n:'②',text:'$T,N,B$는 서로 직교하는 단위벡터이므로$, T·N=T·B=0.$ 내적을 계산할 때 이 성질로 대부분의 항이 소거된다.'}
  ]
},
{
...
검산: $\\left(\\dfrac2{\\sqrt6}\\right)^2+\\left(\\dfrac1{\\sqrt3}\\right)^2=\\dfrac23+\\dfrac13=1$ ✓ ($\\mathbf{w}$가 실제로 접평면 위에 있음을 확인)`},
  ...
  footnotes:[
    {n:'①',text:'회전면 $X(u,v)=(ucosv,usinv,h(u))$의 $u$방향 주곡률$: h\'\'/(1+h\'²)^{3/2}, v$방향 주곡률$: h\'/(u√(1+h\'²)).$'}
  ]
},
...
$$2+(3-a)^2=2\\ \\Rightarrow\\ (3-a)^2=0\\ \\Rightarrow\\ a=3$$`}
  ...
  answer:'$\\tau=0$, $a=3$',
  footnotes:[
    {n:'①',text:'$z=x-1 (y$항이 없는 선형관계)은 $xz$평면에 평행한 평면의 방정식. 이 곡면 위의 모든 곡선은 평면곡선.'}
  ]
},
{
...
$$\\tau=\\frac{(\\gamma'\\times\\gamma'')\\cdot\\gamma'''}{|\\gamma'\\times\\gamma''|^2}=\\frac{(2,0,-3)\\cdot(-1,0,0)}{4+0+9}=\\frac{-2}{13}$$`}
  ...
  footnotes:[
    {n:'①',text:'두 벡터가 평행하면 한 벡터가 다른 벡터의 스칼라배$(k$배)이다. 성분별로 비교해 스칼라 $k$와 미지수를 함께 구한다.'},
...
원의 곡률: $\\kappa=\\dfrac1r=\\dfrac2{\\sqrt3}=\\dfrac{2\\sqrt3}3$`}
  ...
  footnotes:[
    {n:'①',text:'구면(반지름 $R)$ 위 곡선의 일반적 성질$: γ·N=-R²/κ.$ 여기선 $R=1$이므로 $γ·N=-1/κ.$'},
    {n:'②',text:'단위구를 원점에서 거리 $d$인 평면으로 자르면 반지름 $√(1-d²)$인 원이 생긴다 (피타고라스 정리).'}
  ]
},
{
...
$$=\\frac{\\sqrt3\\times2}{4^{3/2}}=\\frac{2\\sqrt3}8=\\frac{\\sqrt3}4$$`}
  ...
  footnotes:[
    {n:'①',text:'정칙곡선(정규곡선): 모든 점에서 속도벡터가 0이 아닌 곡선$. β\'≠0$임을 보이면 충분하다.'},
...
$$\\kappa=\\frac1{(\\sqrt2)^3}=\\frac{\\sqrt2}4$$`}
  ...
  footnotes:[
    {n:'①',text:'$yz$평면은 $x=0$인 평면이므로, 곡선의 매개변수가 $x$일 때 $x=0$을 대입하면 교점을 바로 얻는다.'},
...
$$\\iint_MK\\,dA=\\frac\\pi3\\times\\frac1{\\sqrt2}=\\frac{\\sqrt2\\pi}6$$`}
  ...
  footnotes:[
    {n:'①',text:'$y=ρcosθ,z=ρsinθ$로 놓으면 $z<√3y ⟺ tanθ<√3 ⟺ θ<π/3 (y,z>0$인 경우).'}
  ]
},
...
(단순폐곡선이면 전곡률이 보통 $2\\pi$이지만, 카디오이드는 원점에서 첨점(cusp)을 가지므로 $3\\pi$로 더 큽니다.)`}
  ...
  footnotes:[
    {n:'①',text:'극방정식에서 $θ$를 $θ-c$로 치환하는 것은 곡선 전체를 각 $c$만큼 회전시키는 것과 같아, 길이·곡률 등 회전에 불변인 양은 변하지 않는다.'},
    {n:'②',text:'극곡선 $r=r(θ)$의 곡률$: κ=(r²+2r\'²-rr\'\')/(r²+r\'²)^{3/2}, ds=√(r²+r\'²)dθ$이므로 $κds=(r²+2r\'²-rr\'\')/(r²+r\'²) dθ.$'}
  ]
},
...
$$|\\tau(1)|=\\frac13$$`}
  ...
  footnotes:[
    {n:'①',text:'$α·T=0$을 미분하면 $α\'·T+α·T\'=1+κ(α·N)=0$이 되어 $α·N=-1/κ (α$가 원점 중심 구면에 있지 않아도$, α·T=0$이라는 조건만으로 성립하는 관계식).'}
  ]
},
{
