...
window.SUBJECTS['현대대수학'] = {
  ...
  freqGroups: [
    {
      id:'galois', name:'체론 — 갈루아 이론', stars:'★★★★★★', count:13, color:'#1a4a2e',
    ...
    {
      id:'ring', name:'환론 — 다항식환·아이디얼·잉여환', stars:'★★★★', count:9, color:'#4a1a2d',
...
$$n=\\varphi(38)=\\varphi(2)\\cdot\\varphi(19)=1\\times18=18$$`},
  ...
  answer:'26',
  footnotes:[
    {n:'①',text:'순환군 $G=⟨a⟩$에서 $n$의 약수 $d$마다 부분군 $⟨a^(n/d)⟩$가 하나씩 존재. 개수 $= τ(n) ($약수 개수).'},
    {n:'②',text:'$φ(n): 1~n$ 중 $n$과 서로소인 수의 개수$. φ(p)=p-1, φ(ab)=φ(a)φ(b)($서로소). 순환군에서 위수 $d$인 원소는 $φ(d)$개.'}
  ]
},
{
  id:'2014a3', year:2014, type:'전공A', cat:'ring', pts:4,
  title:'$Z₂₀₁₄[x]$에서 $x²-14$를 두 일차식 곱으로 나타낼 수 없음을 증명',
  problem:`다항식환 $\\mathbb{Z}_{2014}[x]$에서 $f(x)=x^2-14$를 두 일차식의 곱 $f(x)=(ax+b)(cx+d)$로 나타낼 수 없음을 증명하시오.`,
  steps:[
    {num:'Step 1', title:'$WLOG a=c=1$로 환원',
...
따라서 $K=\\mathbb{Q}(\\alpha_0)$.`},
  ...
  footnotes:[
    {n:'①',text:'아이젠슈타인 기준$: f(x)=xⁿ+a_{n-1}xⁿ⁻¹+⋯+a₀$에서 소수 $p$가 모든 $aₖ$를 나누고 $p²∤a₀$이면 ℚ 위 기약.'},
...
$$\\text{ord}((3,3)+H)=1$$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'잉여군 $G/H$에서 항등원 $= H. a∈H ⟺ a+H = H.$'}
  ]
},
...
// ───────────────────────────────────────
{
  ...
  title:'$f(x,y)=9x$의 핵$(kernel)$과 잉여군의 위수',
...
$x\\in\\{0,4,8\\}$, $y$는 자유. $|K|=3\\times6=18$`},
  ...
  answer:'4',
  footnotes:[
    {n:'①',text:'$4|3x$이고 $gcd(3,4)=1$이면 $4|x. (3$의 $mod 4$ 역원은 3이므로 양변에 곱하면 됨.)'},
...
$F=\\mathbb{Q}(\\sqrt{2})\\subset\\mathbb{R}$에서 $i\\notin F$이므로 $\\alpha\\notin F$이고 위 다항식은 기약 ②.`}
  ...
  footnotes:[
    {n:'①',text:'$irr(α,F)$가 $F₀$에서도 기약인 이유$: F₀⊆F$이면 $F₀$에서의 인수분해가 $F$에서도 유효. 만약 $F₀$에서 기약이 아니면 $[K:F]<r$에 모순.'},
    {n:'②',text:'차수 2인 다항식은 근이 있으면 일차 인수로 분해$. F⊂ℝ$이므로 허수 $α∉F,$ 근 없음 → 기약.'}
  ]
},
{
...
$$(3+1)(1+1)=8$$`}
  ...
  answer:'8',
  footnotes:[
    {n:'①',text:'순환군의 부분군: 위수의 약수 $d$마다 하나씩 존재.'},
    {n:'②',text:'갈루아 기본 정리: 중간체 $E ↔$ 부분군 $G(K/E)$의 일대일 대응$. E$가 클수록 대응 부분군은 작음.'}
  ]
},
...
따라서 $|H\\cap N|=8=|H|$이므로 $H\\cap N=H\\implies H\\subseteq N$. $\\blacksquare$`}
  ...
  answer:'$H⊆N ($증명 완료)',
  footnotes:[
    {n:'①',text:'$N◁G, H≤G$이면 $HN$은 $G$의 부분군이고 $|HN|=|H||N|/|H∩N|.$'},
...
$$[F:\\mathbb{Q}]=\\frac{[K:\\mathbb{Q}]}{[K:F]}=\\frac{100}{2}=50$$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'$α$는 $(α+α⁻¹)∈F$를 계수로 갖는 2차 다항식의 근이므로 $[K:F]≤2. σ≠id$이므로 등호 성립.'}
  ]
},
...
// ───────────────────────────────────────
{
  ...
  title:'$Z₁₀×Zₙ$이 순환군이 되는 $n$의 개수 $(10≤n≤100)$',
...
},
{
  ...
  title:'$Z₅[x]$의 아이디얼 $I$의 개수 (위수25, 극대아이디얼 2개)',
...
$$\\text{경우의 수}=\\binom{5}{2}=10$$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'$F[x]/⟨f⟩$의 극대 아이디얼 $↔ f$의 기약 인수$. f=p₁p₂($서로 다른 일차식)이면 극대 아이디얼 2개.'}
  ]
},
{
  id:'2018b6', year:2018, type:'전공B', cat:'galois', pts:5,
  title:'$G(K/Q)$가 아벨군 $⟹ |G|=deg(f), deg=2018$일 때 부분체 개수',
...
$$\\tau(2018)=(1+1)(1+1)=4$$`}
  ...
  answer:'4',
  footnotes:[
    {n:'①',text:'$f$가 기약 $→ G$가 $f$의 근에 전이적$. G$가 아벨이고 전이적이면 단순 추이적$: |G|=$근의 개수$=deg f.$'},
    {n:'②',text:'위수 $pq($서로 다른 소수)인 아벨군은 $Z_{pq}$ 뿐.'},
    {n:'③',text:'갈루아 대응: 부분체 $↔ G$의 부분군. 순환군의 부분군 수 = 위수의 약수 수.'}
  ]
},
...
$$|U|=|U(\\mathbb{Z}_7)|^2=6^2=36$$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'환의 $CRT: gcd(f,g)=1$이면 $F[x]/⟨fg⟩ ≅ F[x]/⟨f⟩ × F[x]/⟨g⟩.$'},
...
$$2\\mid|G(K/\\mathbb{Q})|\\qquad\\blacksquare$$`}
  ...
  footnotes:[
    {n:'①',text:'체에서 비영 원소로 나눌 수 있음$. b≠0$이면 $α≠0$이므로 $α⁻¹∈K.$'},
    {n:'②',text:'$K$가 $F$ 위 갈루아 확대이면 $G(K/F)={F$를 고정하는 $K$의 자기동형사상}.'},
    {n:'③',text:'$|⟨φ⟩|=2$가 $G$의 부분군이므로 라그랑주 정리에 의해 $2∣|G|.$'}
  ]
},
...
// ───────────────────────────────────────
{
  ...
  title:'$S₅$ 원소 $σ$의 위수$, S₅×Z₁₂$의 원소 $(σ,9)$의 위수',
  ...
  steps:[
    {num:'Step 1', title:'$σ$의 순환 표기',
     body:`$1\\to2\\to4\\to1$ (3-순환), $3\\to5\\to3$ (2-순환):

$$\\sigma=(1\\;2\\;4)(3\\;5)$$`},
    {num:'Step 2', title:'$σ$의 위수',
...
},
{
  ...
  title:'$\\varphi_\\alpha$의 핵 $p(x)$, $(x-2)+I$의 역원 $g(x)$ 구하기',
...
$$\\text{irr}(\\zeta,\\mathbb{Q})=\\Phi_{24}(x)=x^8-x^4+1 ③$$`}
  ...
  footnotes:[
    {n:'①',text:'$x²⁴-1$의 분해체 = ℚ(원시 24차 단위근$) = ℚ(ζ₂₄).$'},
    {n:'②',text:'$G(ℚ(ζₙ)/ℚ) ≅ (ℤ/nℤ)*, |G|=φ(n). φ(24)=φ(8)φ(3)=4×2=8.$'},
    {n:'③',text:'원분다항식 $Φₙ(x):$ 원시 $n$차 단위근들을 근으로 갖는 기약 다항식$. Φ₂₄(x)=x⁸-x⁴+1.$'}
  ]
},
...
$$\\frac{72}{6}=12$$`}
  ...
  footnotes:[
    {n:'①',text:'소수 $p$에 대해 $Zₚ*$는 위수 $p-1$인 순환군.'}
  ]
},
{
...
실제로 $\\alpha=2i\\sin(3\\pi/5)$이므로 순허수, $[\\mathbb{Q}(\\alpha):\\mathbb{Q}]=2$.`}
  ...
  footnotes:[
    {n:'①',text:'$ζ₁₀=e^{2πi/10}=e^{iπ/5}∈K. x⁵+5$의 근들로부터 $ζ₁₀$을 만들 수 있음.'},
    {n:'②',text:'$α$의 최소다항식은 더 정밀한 계산 필요$. [ℚ(α):ℚ]=2$임은 $α$가 허수이고 $α²∈ℝ∩ℚ$임을 이용.'}
  ]
},
...
- $8=2^3$: ord$=10/\\gcd(3,10)=10$ ✓`}
  ...
  answer:'2, 6, 7, 8',
  footnotes:[
    {n:'①',text:'순환군 $Z_n$에서 생성원 개수 $= φ(n). φ(10)=φ(2)φ(5)=1×4=4.$'}
  ]
},
{
  id:'2022a12', year:2022, type:'전공A', cat:'galois', pts:4,
  title:'$K=\\mathbb{Q}(\\sqrt[3]{2},e^{2\\pi i/75})$의 갈루아군 위수, $H_1H_2=G$ 증명',
...
$|H_1\\cap H_2|=1$ (계산)이면 $|H_1H_2|=20\\times6=120=|G|$. $\\blacksquare$`}
  ...
  answer:'$|G|=120$',
  footnotes:[
    {n:'①',text:'갈루아 기본 정리에서 $F$가 정규확대 $↔ G(K/F)◁G(K/ℚ).$'}
  ]
},
{
...
**정답 $n=3$** (시험 답안 기준).`}
  ...
  answer:'$n = 3$',
  footnotes:[
    {n:'①',text:'$Z$의 단원은 $±1. Z_{2ⁿ}$의 단원 개수$=φ(2ⁿ)=2^{n-1}. Z₅$의 단원 개수=4.'}
  ]
},
...
// ───────────────────────────────────────
{
  ...
  title:'$x¹³-1$ 분해체$, X$의 원소 개수$, irr(β,Q)$',
...
$$\\text{irr}(\\beta,\\mathbb{Q})=x^2+x-3$$`}
  ...
  footnotes:[
    {n:'①',text:'$G(ℚ(ζₙ)/ℚ) ≅ (Z/nZ)*. n=13$이면 $(Z/13Z)*≅Z₁₂.$'},
...
$$\\text{char}=5$$`}
  ...
  answer:'$\\min\\nu(\\eta)=5$, $\\text{char}=5$',
  footnotes:[
    {n:'①',text:'$Z[i]$는 유클리드 정역$: β=qα+r$에서 $q=β/α$를 계산하고 가장 가까운 가우스 정수로 반올림.'},
    {n:'②',text:'$Z[i]/⟨π⟩ ≅ F_{ν(π)} (π$가 가우스 소수일 때$). ν(2-i)=5$는 소수 $→ Z[i]/⟨2-i⟩ ≅ F₅.$'}
  ]
},
...
// ───────────────────────────────────────
{
  ...
  title:'순환군 잉여군 생성원 개수$, G/H=(K/H)(L/H)$ 만족 $L$의 개수',
...
$5,10,20,40,65,130,260,520$ → $8$개.`}
  ...
  answer:'생성원: 192개 $/ L$의 개수: 8',
  footnotes:[
    {n:'①',text:'순환군 $G$에서 $(A)(B)=G ⟺ lcm(|A|,|B|)=|G|.$'}
  ]
},
{
  id:'2024a12', year:2024, type:'전공A', cat:'galois', pts:4,
  title:'$x²³-88$의 분해체 $[K:Q],$ 체 $E$의 개수',
...
(최종 답: 추가 계산 필요)`}
  ...
  footnotes:[
    {n:'①',text:'$x²³-a$에서 23이 소수이고 $a$가 23제곱수가 아니면 분해체 $[K:Q]=23×φ(23)=23×22=506.$'}
  ]
},
{
...
$(5,2),(5,3),(5,4)$ ✓`}
  ...
  footnotes:[
    {n:'①',text:'$F[x]/⟨f⟩$가 정역 $⟺ f$가 기약 $(F$가 체일 때).'},
    {n:'②',text:'$x²+bx+c$가 기약 ⟺ 판별식 $b²-4c$가 이차비잉여.'}
  ]
},
...
// ───────────────────────────────────────
{
  ...
  title:'표수 $a$인 체 $F, G=Z₄×F*$ 순환군 조건$, a$와 $b$의 값',
...
$n=1,2,3,4,5$: $b=5$개 (모두 $\\gcd(4,2^n-1)=1$ 만족).`}
  ...
  answer:'$a=2$, $b=5$',
  footnotes:[
    {n:'①',text:'$Zₘ×Zₙ$이 순환군 $⟺ gcd(m,n)=1. |F*|=|F|-1.$'}
  ]
},
{
  id:'2025b8', year:2025, type:'전공B', cat:'galois', pts:4,
  title:'$G(K/Q)≅Z₂×S₃, [E:Q]=6$인 부분체 개수$, S₃$ 동형 부분체 존재',
  ...
  steps:[
    {num:'Step 1', title:'$[E:Q]=6$인 부분체 개수',
...
$$G(F/\\mathbb{Q})\\cong G/N\\cong S_3\\qquad\\blacksquare$$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'중간체 $F$가 갈루아 확대 $⟺ G(K/F)◁G. G/G(K/F) ≅ G(F/ℚ).$'}
  ]
},
{
...
$$S\\cong\\mathbb{Z}_{12}/K_2\\in\\{\\mathbb{Z}_4,\\mathbb{Z}_2,\\{0\\}\\}$$`}
  ...
  footnotes:[
    {n:'①',text:'$gcd(3,10)=1$이므로 $⟨3⟩=Z₁₀. gcd(8,12)=4$이므로 8을 포함하는 최소 아이디얼은 ⟨4⟩.'}
  ]
},
...
$$|\\ker\\phi|=26$$`}
  ...
  footnotes:[
    {n:'①',text:'$Zd$의 부분군 개수$=τ(d)($약수 개수)=6. 200의 약수 중 약수가 6개인 것: 20=2²×5, 50=2×5².'},
    {n:'②',text:'준동형 정리$: |G|=|Im||Ker|.$'}
  ]
},
{
  id:'2026b9', year:2026, type:'전공B', cat:'galois', pts:4,
  title:'$Z₇[x]$에서 $x⁴+3x²-1$ 기약 증명, 고정체 $E$의 위수',
