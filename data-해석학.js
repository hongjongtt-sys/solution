...
따라서 $f(x^*) = M$. $\\blacksquare$`}
  ...
  footnotes:[
    {n:'①',text:'상한의 정의: M이 상한이면 M보다 작은 어떤 수도 상한이 아님. 따라서 모든 n에 대해 M-1/n은 상한이 아니므로, M-1/n < f(xₙ)인 xₙ이 존재.'},
    {n:'②',text:'볼차노-바이어슈트라스(Bolzano-Weierstrass) 정리: 모든 유계인 실수열은 수렴하는 부분수열을 가짐. 고등학교 수열과 같은 개념의 깊은 버전.'},
    {n:'③',text:'조임 정리(Squeeze theorem): M-1/nₖ < f(xₙₖ) ≤ M이고 M-1/nₖ → M이므로 f(xₙₖ) → M.'}
  ]
},
...
$t=\\frac{1}{2}$일 때: $c = x\\!\\left(\\frac{1}{2}\\right) = 4\\cdot\\frac{1}{2} - \\left(\\frac{1}{2}\\right)^2 = 2 - \\frac{1}{4} = \\frac{7}{4}$`}
  ...
  answer:'$c = \\dfrac{7}{4}$',
  footnotes:[
    {n:'①',text:'평균값 정리(MVT): f가 [a,b]에서 연속, (a,b)에서 미분가능하면 f(b)-f(a)=(b-a)f\'(c)인 c∈(a,b)가 존재.'},
...
$\\therefore f'(c)=1$. 그런데 $|f'(c)|\\leq M<1$. 모순. $\\blacksquare$`}
  ...
  footnotes:[
    {n:'①',text:'평균값 정리(MVT): f가 [0,x]에서 연속, (0,x)에서 미분가능이면 f(x)-f(0)=f\'(c)x인 c∈(0,x) 존재.'},
    {n:'②',text:'사이값 정리(IVT): g가 연속이고 g(0)>0, g(x₀)<0이면 g(x*)=0인 x*∈(0,x₀) 존재.'}
  ]
},
{
...
$\\sum f_n'(x)$도 균등수렴. 균등수렴 미분 정리 ②에 의해 $\\sum f_n$은 미분가능. $\\blacksquare$`}
  ...
  footnotes:[
    {n:'①',text:'Weierstrass M-판정법: |fₙ(x)| ≤ Mₙ (모든 x에 대해)이고 ΣMₙ < ∞이면 Σfₙ은 균등수렴.'},
    {n:'②',text:'균등수렴 미분 정리: Σfₙ이 어느 한 점에서 수렴하고, Σfₙ\'이 균등수렴하면, Σfₙ도 균등수렴하고 (Σfₙ)\' = Σfₙ\'.'}
  ]
},
...
$$\\lim_{n\\to\\infty}\\int_0^1 g_n(x)\\,dx = \\int_0^1 f'(x)\\,dx = f(1)-f(0)\\qquad\\blacksquare$$`}
  ...
  footnotes:[
    {n:'①',text:'MVT: f(x+h)-f(x)=f\'(c)h인 c∈(x,x+h) 존재. 따라서 gₙ(x)=f\'(cₙ,ₓ).'},
    {n:'②',text:'컴팩트 집합([0,2])에서 연속함수는 균등연속. 즉 |x-y|<δ이면 |f\'(x)-f\'(y)|<ε인 δ가 x,y에 무관하게 존재.'},
    {n:'③',text:'균등수렴과 적분 교환 정리: fₙ→f 균등수렴, 각 fₙ이 연속이면 ∫limfₙ = lim∫fₙ.'}
  ]
},
{
...
따라서 $f\\equiv 0$ on $\\{|x|\\leq 2\\}$. 이를 확장하면 $f\\equiv 0$ on $\\mathbb{R}$. 상수함수가 아니라는 가정에 모순. $\\blacksquare$`}
  ...
  footnotes:[
    {n:'①',text:'귀납 논증: f(xₖ)=0이고 xₖ→c이면 f(c)=0. 그다음 f\'(c)= lim(f(xₖ)-f(c))/(xₖ-c)=0. 같은 방식으로 f\'(c)=0이고 f\'(xₖ)→f\'(c)=0이므로 f\"(c)=0. 이를 반복.'},
    {n:'②',text:'라그랑주 나머지: f(x) = Σ f⁽ᵏ⁾(c)/k! (x-c)ᵏ + f⁽ⁿ⁾(tₓ)/n! (x-c)ⁿ. 여기서 tₓ는 c와 x 사이의 점. 모든 f⁽ᵏ⁾(c)=0이면 첫 항이 사라짐.'}
  ]
},
...
이 문제는 극한과 적분이 교환되지 않는 예시 ②이다.`}
  ...
  answer:'$1$',
  footnotes:[
    {n:'①',text:'fₙ이 양수인 구간: |x-2n|<n, 즉 n<x<3n. 이 구간에서 텐트(tent) 함수 모양. 높이=1/n, 밑변=2n이므로 넓이=1.'},
    {n:'②',text:'lim∫fₙ=1 ≠ 0=∫lim fₙ. 균등수렴이 아닐 때 이런 역설이 발생. fₙ이 ∞으로 이동하므로 균등수렴 불성립.'}
  ]
},
{
...
$$\\lim_{x\\to 0}\\frac{f(x)\\cos(1/x^2)}{x} = 0$$`},
  ...
  answer:"$g'(0) = 0$",
  footnotes:[
    {n:'①',text:'핵심 아이디어: f\'(0)=0이면 lim_{x→0} f(x)/x = 0. cos(1/x²)는 유계이므로 (f(x)/x)·cos(1/x²)→0.'}
  ]
},
{
...
이것은 리프시츠 조건 ③이므로 $f$는 균등연속. $\\blacksquare$`}
  ...
  footnotes:[
    {n:'①',text:'|arctan(t)| ≤ |t|: arctan(0)=0이고 (arctan)\'(t)=1/(1+t²)≤1이므로 MVT에 의해 |arctan(t)-arctan(0)| ≤ |t|.'},
    {n:'②',text:'|arctan(x/n)-arctan(y/n)| ≤ |(x-y)/n|: MVT를 arctan에 적용. 도함수=1/(1+t²)≤1.'},
    {n:'③',text:'리프시츠(Lipschitz) 조건: |f(x)-f(y)| ≤ L|x-y|이면 f는 균등연속. L=π²/6.'}
  ]
},
...
따라서 $c\\in\\mathbb{R}$에서 $f(c)=0$이고 $f'(c)=0$. 대우 증명 완료 → $P$ 증명. $\\blacksquare$`}
  ...
  footnotes:[
    {n:'①',text:'대우 증명: 명제 "P이면 Q"를 증명하는 대신 "not Q이면 not P"를 증명하는 방법. 논리적으로 동치.'},
    {n:'②',text:'f\'(c)=lim(f(xₖ)-f(c))/(xₖ-c): 수열을 이용한 미분 정의. xₖ→c이고 xₖ≠c이므로 극한값이 도함수.'}
  ]
},
{
...
$$\\lim_{n\\to\\infty}a_n = \\int_0^1 g(x)\\,dx = \\int_0^1 x\\,dx = \\frac{1}{2}$$`}
  ...
  footnotes:[
    {n:'①',text:'AM-GM: x,y≥0이면 (x-y)·sin(xy)의 최대화. x∈[0,1], y∈[0,x]에서 (x-y)·xy ≤ (x·y)의 최대 = 1/4 (x²/2에서 최대화 등).'}
  ]
},
...
따라서 $f'(c)=1$. 가정 ($f'(x)\\neq 1$ on $(0,1)$)에 모순. $\\blacksquare$`}
  ...
  footnotes:[
    {n:'①',text:'사이값 정리(IVT): g(0)≥0이고 g(1)≤0이면 (등호 성립 시 바로 고정점 발견) 또는 엄격 부등호면 IVT 적용.'}
  ]
},
...
`}
  ...
  footnotes:[
    {n:'①',text:'등비급수를 반복 미분하여 멱급수 계수를 구하는 방법. 1/(1-x)=Σxⁿ을 미분하면 1/(1-x)²=Σ(n+1)xⁿ. 한번 더 미분하면 2/(1-x)³=Σ(n+1)(n+2)xⁿ.'},
    {n:'②',text:'수렴반경 |x|<1이고 |1/3|<1이므로 x=1/3 대입 가능.'}
  ]
},
{
...
$\\sum e^{-n} = \\dfrac{1}{e-1}<\\infty$ → Weierstrass M-판정법 ①으로 균등수렴. ✓`}
  ...
  footnotes:[
    {n:'①',text:'Mₙ ~ e⁻ⁿ이므로 ΣMₙ ~ Σe⁻ⁿ이 수렴. |fₙ(x)| ≤ Mₙ이므로 Weierstrass에 의해 균등수렴.'}
  ]
},
{
...
$$= \\left[\\frac{t^2}{2}\\right]_{\\pi}^{3\\pi/2} = \\frac{9\\pi^2}{8} - \\frac{\\pi^2}{2} = \\frac{5\\pi^2}{8}$$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'1+tan²t = sec²t이므로 1/(1+tan²t)·sec²t = 1. 따라서 적분 ∫g(x)/(1+x²)dx = ∫t dt (치환 후).'}
  ]
},
...
$$\\boxed{\\alpha > 2}$$`}
  ...
  footnotes:[
    {n:'①',text:'u=x, dv=x(1-x²)ⁿdx로 부분적분. ∫x(1-x²)ⁿdx = -1/(2(n+1))·(1-x²)^{n+1}+C.'},
...
$f_n(0)=0$이고 $f_n\\to f$ 균등이므로 적분값이 수렴. 답: $\\displaystyle\\int_0^1 f(x)dx$.`}
  ...
  answer:'균등수렴 ✓',
  footnotes:[
    {n:'①',text:'x∈[0,∞)에서 xe⁻ˣ ≤ 1/e < 1이고 1-xe⁻ˣ ≥ 1-1/e > 0이므로 (1-e⁻ˣ)/(1-xe⁻ˣ) ≤ 1/(1-1/e)로 유계.'}
  ]
},
...
$f$는 $(0,\\infty)$에서 연속이고, 사이값 정리 ①에 의해 $f(x_0)=L$인 $x_0\\in(y_n, x_n)$ 존재. $\\blacksquare$`}
  ...
  footnotes:[
    {n:'①',text:'중간값 정리(IVT): f가 [a,b]에서 연속이고 f(a)와 f(b) 사이의 값 L에 대해 f(x₀)=L인 x₀∈(a,b) 존재.'}
  ]
},
{
...
따라서 $a_n\\approx\\dfrac{1}{n^2}$, $\\displaystyle\\sum a_n = \\frac{\\pi^2}{6}$.`}
  ...
  footnotes:[
    {n:'①',text:'Weierstrass M-판정법: |fₙ(x)| ≤ Mₙ=1/n²이고 Σ1/n²=π²/6이면 Σfₙ 균등수렴.'}
  ]
},
...
르베그 수렴 정리 ②로 $\\displaystyle\\lim_{s\\to 0^+}\\int_0^{1/s}e^{-1/(ts)^2-t^2}dt = \\int_0^{\\infty}0\\cdot e^{-t^2}dt = 0$`}
  ...
  footnotes:[
    {n:'①',text:'x→0+: e^{-1/x²}→0이 x→0보다 훨씬 빠름. x→∞: e^{-1/x²}→1이지만 1/x→0. 따라서 양쪽 극한 모두 0.'},
    {n:'②',text:'각 t>0에 대해 e^{-1/(ts)²}→0 (s→0+). 피적분함수가 e^{-t²}으로 적분가능한 함수에 의해 지배되므로 우세수렴 정리 적용.'}
  ]
}
