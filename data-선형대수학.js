...
가장 작은 자연수: $a=15$`}
  ...
  answer:'$15$',
  footnotes:[
    {n:'①',text:'도형을 각 $φ$만큼 시계방향 회전시키는 것은, 새 좌표축 관점에서 옛 좌표를 반시계방향 $φ$ 회전으로 표현하는 것과 같다 (좌표축 회전 공식).'},
    {n:'②',text:'이차형식의 $xy$ 교차항이 소거되는 각도로 회전시키는 것이 이차곡선을 표준형으로 만드는 표준적 기법.'},
    {n:'③',text:'$AX²+BY²+(1$차항$)+C=0$에서 $AB<0$이면 쌍곡선$, AB>0$이면 타원(또는 원$), AB=0$이면 포물선(또는 퇴화).'}
  ]
},
...
$$=\\lambda^3+\\lambda^2-\\lambda-1$$`},
  ...
  footnotes:[
    ...
    {n:'③',text:'축 방향 단위벡터 $u$에 대한 180° 회전 행렬$: R=2uu^T-I. uu^T$는 $u$방향으로의 정사영 행렬.'}
  ]
},
...
$k=-1$: $\\text{diag}(1,0,0)$ → 0이 아닌 대각성분 1개 → $\\text{rank}=1$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'일반적인(직교하지 않는) 기저에 대한 정사영 행렬$: P=V(VᵀV)⁻¹Vᵀ (V$는 기저벡터를 열로 하는 행렬).'}
  ]
},
...
$$a_{11}+a_{12}+a_{13}=1$$`}
  ...
  footnotes:[
    ...
    {n:'②',text:'$A(1,1,1)$의 첫 성분은 $A$의 첫 행과 (1,1,1)의 내적, 즉 $a11·1+a12·1+a13·1=a11+a12+a13.$'}
  ]
},
...
**대각화가능**`}
  ...
  footnotes:[
    {n:'①',text:'$[L]_α$의 $j$번째 열은 $L(α$의 $j$번째 기저벡터)를 $α$로 나타낸 좌표벡터.'},
    {n:'②',text:'삼각행렬의 고유값은 대각성분과 일치$. n×n$ 행렬이 서로 다른 $n$개의 고유값을 가지면 항상 대각화가능.'}
  ]
},
...
$$\\det(A)=|\\alpha|^2\\times3=2\\times3=6$$`}
  ...
  footnotes:[
    {n:'①',text:'$A-3I$가 역행렬을 갖지 않는다는 것은 $det(A-3I)=0,$ 즉 3이 $A$의 고유값이라는 뜻.'},
    {n:'②',text:'실계수 다항식(여기선 특성다항식)의 복소근은 항상 켤레쌍으로 나타난다.'},
    {n:'③',text:'최소다항식의 차수가 특성다항식의 차수보다 낮다는 것은, 어떤 고유값이 여러 개의 서로 다른$(A$의) 고유값으로부터 $B$에서 같은 값으로 합쳐졌거나, 대수적 중복도가 있으면서 최소다항식에서 1제곱만 나타나는 경우이다.'}
  ]
},
...
$$P=\\begin{pmatrix}\\frac1{\\sqrt6}&\\frac1{\\sqrt2}&\\frac1{\\sqrt3}\\\\-\\frac2{\\sqrt6}&0&\\frac1{\\sqrt3}\\\\\\frac1{\\sqrt6}&-\\frac1{\\sqrt2}&\\frac1{\\sqrt3}\\end{pmatrix}$$`}
  ...
  footnotes:[
    {n:'①',text:'$A⁻¹v=(1/2)v$ 양변에 $A$를 곱하면 $v=(1/2)Av,$ 즉 $Av=2v.$'},
...
$$\\text{정규직교기저: }\\left\\{\\frac{(1,2,2)}3,\\ \\frac{(2,-2,1)}3,\\ \\frac{(2,1,-2)}3\\right\\}$$`}
  ...
  footnotes:[
    {n:'①',text:'$B=cA⁻¹$은 $A$와 같은 고유벡터를 가지는데$, B$가 대칭행렬이려면 서로 다른 고유값의 고유벡터가 직교해야 한다 — 따라서 $A$의 이 두 고유벡터도 애초에 직교해야 한다.'},
    {n:'②',text:'세 번째 고유벡터도 앞의 두 벡터와 직교해야 하므로(대칭행렬 $B$의 고유벡터 조건), 외적으로 유일하게 결정된다.'},
    {n:'③',text:'$B=cA⁻¹$이므로 $A$의 임의의 고유벡터 $v (Av=λv)$에 대해 $A⁻¹v=(1/λ)v,$ 즉 $Bv=(c/λ)v — B$의 고유벡터가 곧 $A$의 고유벡터.'}
  ]
}
